...
- Makefile
    ...
    2. **소스 파일(.c)** --using gcc compiler-→ **목적 파일(.o)**: 기계가 이해할 수 있는 언어 → **실행 파일(a.out)**
    ...
    13. **CFLAGS**(컴파일.o라는 목적 파일 만들기할 때 옵션을 줄 수 있음) **= -Wall**
        ```c
        $(CC) $(CFLAGS) -c -o $@ $<
        ```

    14. **LDFLAGS**(링크.o파일을 바이너리(타겟) 개념으로 만듦할 때 옵션을 줄 수 있음) **= -lc**(library) << 좀 더 찾아보기!!!!!
        ```c
        $(CC) $(LDFLAGS) -o $@ $^*
        ```
